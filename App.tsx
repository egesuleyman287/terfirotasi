...
export default function App() {
  ...
  function checkOrNext() {
    ...
    setIndex(index + 1); setSelected(null); setChecked(false);
  }
  ...
  const Header = useMemo(() => <View style={{ backgroundColor: COLORS.white, borderBottomWidth: 1, borderBottomColor: COLORS.line, paddingHorizontal: 20, paddingVertical: 10, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 26 }}><Pressable onPress={() => setScreen('home')} style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}><Image source={TERFI_AMBLEM} style={{ width: 44, height: 44, resizeMode: 'contain' }} /><View><Text style={{ color: COLORS.navy, fontSize: 21, fontWeight: '900' }}>TERFİ <Text style={{ color: COLORS.red }}>ROTASI</Text></Text><Text style={{ color: COLORS.muted, fontSize: 10, marginTop: 2 }}>SINAV HAZIRLIK PLATFORMU</Text></View></Pressable><View style={{ flexDirection: 'row', gap: 18, alignItems: 'center' }}><Pressable onPress={() => setScreen('home')}><Text style={{ color: screen === 'home' ? COLORS.blue : COLORS.ink, fontSize: 15, fontWeight: '800' }}>Ana Sayfa</Text></Pressable><Pressable onPress={() => setScreen('exams')}><Text style={{ color: screen === 'exams' ? COLORS.blue : COLORS.ink, fontSize: 15, fontWeight: '800' }}>Sınavlar</Text></Pressable><Pressable onPress={() => setScreen('membership')}><Text style={{ color: screen === 'membership' ? COLORS.blue : COLORS.ink, fontSize: 15, fontWeight: '800' }}>Paketler</Text></Pressable></View></View><View style={{ flexDirection: 'row', gap: 10, alignItems: 'center' }}>{screen === 'quiz' && studyMode === 'mock' && <Text style={{ color: COLORS.red, fontWeight: '800' }}>{timerLabel}</Text>}{user ? <Pressable onPress={() => setScreen('auth')} style={{ backgroundColor: '#EAF5FA', borderRadius: 7, paddingHorizontal: 14, paddingVertical: 9 }}><Text style={{ color: COLORS.blue, fontSize: 13, fontWeight: '800' }}>Hesabım</Text></Pressable> : <><Pressable onPress={() => { setAuthMode('login'); setAuthFeedback(null); setScreen('auth'); }} style={{ borderWidth: 1, borderColor: COLORS.blue, borderRadius: 7, paddingHorizontal: 14, paddingVertical: 9 }}><Text style={{ color: COLORS.blue, fontSize: 13, fontWeight: '800' }}>Giriş Yap</Text></Pressable><Pressable onPress={() => { setAuthMode('signup'); setAuthFeedback(null); setScreen('auth'); }} style={{ backgroundColor: COLORS.blue, borderRadius: 7, paddingHorizontal: 14, paddingVertical: 9 }}><Text style={{ color: COLORS.white, fontSize: 13, fontWeight: '800' }}>Üye Ol</Text></Pressable></>}</View></View>, [screen, user, studyMode, timerLabel]);
