  ...
  function Home() {
    ...
    return <ScrollView contentContainerStyle={[styles.page, { gap: compactHeader ? 14 : 20 }]}>
      <View style={{ backgroundColor: '#D9F1F6', borderRadius: 18, padding: compactHeader ? 20 : 42, flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: compactHeader ? 20 : 42, overflow: 'hidden' }}>
        <View style={{ flex: 1, minWidth: compactHeader ? 0 : 420, gap: compactHeader ? 10 : 15 }}><Text style={{ color: COLORS.red, fontSize: compactHeader ? 22 : 31, fontWeight: '900' }}>TCDD Taşımacılık A.Ş.</Text><Text style={{ color: '#101A22', fontSize: compactHeader ? 27 : 40, fontWeight: '900', lineHeight: compactHeader ? 35 : 49 }}>Görevde Yükselme{`\n`}Sınav Hazırlığı</Text><Text style={{ color: COLORS.blue, fontSize: compactHeader ? 16 : 20, fontWeight: '800' }}>Kayıtlar başladı.</Text><Pressable onPress={goToStart} style={{ alignSelf: 'flex-start', backgroundColor: COLORS.blue, paddingHorizontal: 25, paddingVertical: 14, borderRadius: 9, marginTop: 5 }}><Text style={{ color: COLORS.white, fontWeight: '900' }}>{user ? 'ÇALIŞMAYA BAŞLA' : 'ÜCRETSİZ BAŞLA'}</Text></Pressable></View>
        <View style={{ flex: 1, minWidth: compactHeader ? '100%' : 330, backgroundColor: COLORS.white, borderRadius: 16, minHeight: compactHeader ? 210 : 310, alignItems: 'center', justifyContent: 'center', padding: 22 }}><Image source={TCDD_LOGO} style={{ width: compactHeader ? 190 : 280, height: compactHeader ? 170 : 260, resizeMode: 'contain' }} /></View>
      </View>
