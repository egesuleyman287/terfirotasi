...
const VHKİ_TOPICS = ['Türkçe ve Dil Bilgisi', '1982 Türkiye Cumhuriyeti Anayasası', 'Atatürk İlkeleri ve İnkılap Tarihi', '657 Sayılı Devlet Memurları Kanunu', '1 Sayılı Cumhurbaşkanlığı Teşkilatı Hakkında Cumhurbaşkanlığı Kararnamesi (İçişleri Bakanlığı)', '4982 Sayılı Bilgi Edinme Hakkı Kanunu', '3071 Sayılı Dilekçe Hakkının Kullanılmasına Dair Kanun', '7315 Sayılı Güvenlik Soruşturması ve Arşiv Araştırması Kanunu', 'İçişleri Bakanlığı Personeli Görevde Yükselme ve Unvan Değişikliği Yönetmeliği', 'İçişleri Bakanlığı Personeli Atama ve Yer Değiştirme Yönetmeliği', 'Devlet Memurları Disiplin Yönetmeliği', 'İçişleri Bakanlığı Disiplin Amirleri Yönetmeliği', 'Mal Bildiriminde Bulunulması Hakkında Yönetmelik', 'Resmî Yazışmalarda Uygulanacak Usul ve Esaslar Hakkında Yönetmelik'];
const SUBE_MUDURU_TOPICS = ['Valilik ve Kaymakamlık Birimleri Teşkilat, Görev ve Çalışma Yönetmeliği', 'Resmî Yazışmalarda Uygulanacak Usul ve Esaslar Hakkında Yönetmelik', 'İçişleri Bakanlığı Disiplin Amirleri Yönetmeliği', 'Devlet Memurları Disiplin Yönetmeliği', '7315 Sayılı Güvenlik Soruşturması ve Arşiv Araştırması Kanunu', '3071 Sayılı Dilekçe Hakkının Kullanılmasına Dair Kanun', '4982 Sayılı Bilgi Edinme Hakkı Kanunu', '1 Sayılı Cumhurbaşkanlığı Teşkilatı Hakkında Cumhurbaşkanlığı Kararnamesi (İçişleri Bakanlığı)', '657 Sayılı Devlet Memurları Kanunu', 'Atatürk İlkeleri ve İnkılap Tarihi', '1982 Türkiye Cumhuriyeti Anayasası', 'Türkçe ve Dil Bilgisi', 'Mal Bildiriminde Bulunulması Hakkında Yönetmelik', '5442 Sayılı İl İdaresi Kanunu', '3628 Sayılı Mal Bildiriminde Bulunulması, Rüşvet ve Yolsuzluklarla Mücadele Kanunu', '2577 Sayılı İdari Yargılama Usulü Kanunu', '4483 Sayılı Memurlar ve Diğer Kamu Görevlilerinin Yargılanması Hakkında Kanun', '5018 Sayılı Kamu Mali Yönetimi ve Kontrol Kanunu', '5651 Sayılı İnternet Ortamında Yapılan Yayınların Düzenlenmesi ve Bu Yayınlar Yoluyla İşlenen Suçlarla Mücadele Edilmesi Hakkında Kanun', 'Telekomünikasyon Kurumu Tarafından Erişim Sağlayıcılara ve Yer Sağlayıcılara Faaliyet Belgesi Verilmesine İlişkin Usul ve Esaslar Hakkında Yönetmelik', 'İnternet Toplu Kullanım Sağlayıcıları Hakkında Yönetmelik', 'İnternet Ortamında Yapılan Yayınların Düzenlenmesine Dair Usul ve Esaslar Hakkında Yönetmelik', '7201 Sayılı Tebligat Kanunu', 'İçişleri Bakanlığı Merkez ve Taşra Teşkilatı Arşiv Hizmetleri Yönetmeliği', 'İçişleri Bakanlığı Personeli Atama ve Yer Değiştirme Yönetmeliği', 'İçişleri Bakanlığı Personeli Görevde Yükselme ve Unvan Değişikliği Yönetmeliği', 'Memurların Yetiştirilmeleri ve Eğitimleri Hakkında Genel Yönetmelik'];
const TCDD_ROLES: RoleDefinition[] = ROLES.map(role => ({ name: role.name, special: role.special, topics: [...COMMON_TOPICS, SPECIAL_TOPICS[role.name]] }));
const ICISLERI_ROLES: RoleDefinition[] = [
  { name: 'VHKİ', special: '14 konu', topics: VHKİ_TOPICS },
  { name: 'Şube Müdürü', special: '27 konu', topics: SUBE_MUDURU_TOPICS },
];
const INSTITUTIONS: Institution[] = [
  { id: 'tcdd', name: 'TCDD Taşımacılık A.Ş.', detail: 'Görevde yükselme sınavları', logo: TCDD_LOGO, roles: TCDD_ROLES },
  { id: 'icisleri', name: 'T.C. İçişleri Bakanlığı', detail: 'Görevde yükselme ve unvan değişikliği', mark: 'T.C.', roles: ICISLERI_ROLES },
];
function institutionForRole(role: Role) { return INSTITUTIONS.find(institution => institution.roles.some(item => item.name === role)) ?? INSTITUTIONS[0]; }
function roleDefinition(role: Role) { return institutionForRole(role).roles.find(item => item.name === role) ?? TCDD_ROLES[0]; }
function topicsForRole(role: Role) { return roleDefinition(role).topics; }
let QUESTIONS: Question[] = SEED_QUESTIONS_399;
function makeSecurityCode() { return Math.random().toString(36).slice(2, 7).toUpperCase(); }
...
export default function App() {
  ...
  const [authInstitutionOpen, setAuthInstitutionOpen] = useState(false);
  const [authRoleOpen, setAuthRoleOpen] = useState(false);
  const [authRoleSelected, setAuthRoleSelected] = useState(false);
  const [institutionOpen, setInstitutionOpen] = useState(false);
  const [selectedInstitutionId, setSelectedInstitutionId] = useState<InstitutionId | ''>('');
  const [homeInstitutionPage, setHomeInstitutionPage] = useState(0);
  ...
  const [securityCode, setSecurityCode] = useState(makeSecurityCode);
  const [securityEntry, setSecurityEntry] = useState('');
  ...
  useEffect(() => {
    ...
    setPasswordResetToken(params.get('access_token') ?? '');
    setNewPassword(''); setNewPasswordConfirm(''); setResetFeedback(null); setScreen('reset-password');
  ...
  function openStudy(nextRole: Role) { if (!user) { setAuthInstitutionId(institutionForRole(nextRole).id); setAuthRole(nextRole); setAuthRoleSelected(true); setAuthMode('signup'); setEmailVerificationPending(false); setAuthFeedback(null); Alert.alert('Ücretsiz üyelik oluştur', 'Sorulara başlamadan önce ücretsiz hesabını oluşturmalısın.'); setScreen('auth'); return; } setStudyRole(nextRole); setStudyTopic(topicsForRole(nextRole)[0]); setTopicsOpen(false); setStudyMode('topic'); setQuestionCount(10); setScreen('study'); }
  ...
  function questionPoolFor(nextRole: Role, nextTopic: string) {
    const isTcddRole = institutionForRole(nextRole).id === 'tcdd';
    const common = isTcddRole ? (COMMON_QUESTION_CATALOG[nextTopic as keyof typeof COMMON_QUESTION_CATALOG] ?? []) : [];
    const uploaded = storedQuestions
      .filter(question => question.topic === nextTopic && (isTcddRole && COMMON_TOPICS.includes(nextTopic) || question.role === nextRole))
  ...
  function allQuestionPoolFor(nextRole: Role) {
    const isTcddRole = institutionForRole(nextRole).id === 'tcdd';
    const common = isTcddRole ? Object.values(COMMON_QUESTION_CATALOG).flat() as Question[] : [];
    const uploaded = storedQuestions
      .filter(question => (isTcddRole && COMMON_TOPICS.includes(question.topic)) || question.role === nextRole)
      .map(question => ({ topic: question.topic, text: question.text, choices: question.choices, answer: question.correctAnswer, reference: uploadedReference(question.topic, question.text, question.reference), explanation: 'Bu soru yönetici tarafından soru havuzuna eklenmiştir.' }));
  ...
  async function signInRemote() {
    ...
    if (authMode === 'signup' && (!authName.trim() || !authInstitutionId || !authRoleSelected || !authCity.trim() || authPhone.replace(/\D/g, '').length < 10)) { setAuthFeedback({ type: 'error', text: 'Ad soyad, kurum, unvan, il ve en az 10 haneli telefon numarası zorunludur.' }); return; }
    ...
    if (authMode === 'signup' && securityEntry.trim().toLocaleUpperCase('tr-TR') !== securityCode) { setAuthFeedback({ type: 'error', text: 'Güvenlik kodu hatalı. Kodu kontrol edip tekrar dene.' }); setSecurityEntry(''); setSecurityCode(makeSecurityCode()); return; }
    ...
    try {
      const signedUser = authMode === 'signup'
        ? await createRemoteAccount({ name: authName.trim(), email: authEmail.trim().toLowerCase(), institution: INSTITUTIONS.find(item => item.id === authInstitutionId)?.name, role: authRole, city: authCity.trim(), phone: authPhone.trim() }, authPassword)
      ...
      const signedRole = (signedUser.role as Role) ?? authRole; setStudyRole(signedRole); setStudyTopic(topicsForRole(signedRole)[0]); setTopicsOpen(false); setStudyMode('topic'); setQuestionCount(10); setScreen('home');
  ...
  async function signIn() {
    ...
    setUser(signedUser); setPlan('free'); await AsyncStorage.setItem('terfi_plan', 'free'); setStudyRole(authRole); setStudyTopic(topicsForRole(authRole)[0]); setTopicsOpen(false); setStudyMode('topic'); setQuestionCount(10); setScreen('study'); Alert.alert('Ücretsiz üyeliğin oluşturuldu', `${signedUser.name}, şimdi konunu seçip çalışmaya başlayabilirsin.`);
  }
  ...
  async function completePasswordReset() {
    if (resetBusy) return;
    if (newPassword.length < 6) { setResetFeedback({ type: 'error', text: 'Yeni şifren en az 6 karakter olmalıdır.' }); return; }
    if (newPassword !== newPasswordConfirm) { setResetFeedback({ type: 'error', text: 'Şifreler eşleşmiyor. Lütfen iki alana da aynı şifreyi yaz.' }); return; }
    if (!passwordResetToken) { setResetFeedback({ type: 'error', text: 'Şifre yenileme bağlantısı geçersiz veya süresi dolmuş. Lütfen yeniden istek oluştur.' }); return; }
    setResetBusy(true); setResetFeedback(null);
    try {
      await updateRemotePassword(passwordResetToken, newPassword);
      setNewPassword(''); setNewPasswordConfirm(''); setPasswordResetToken('');
      setResetFeedback({ type: 'success', text: 'Şifren başarıyla yenilendi. Yeni şifrenle giriş yapabilirsin.' });
      setTimeout(() => { setAuthMode('login'); setScreen('auth'); }, 1300);
    } catch (error) {
      setResetFeedback({ type: 'error', text: error instanceof Error ? error.message : 'Şifre yenilenemedi. Lütfen yeniden dene.' });
  ...
  function Home() {
    ...
    const homeInstitution = INSTITUTIONS[homeInstitutionPage];
    const isTcddHome = homeInstitution.id === 'tcdd';
    return <ScrollView contentContainerStyle={[styles.page, { gap: compactHeader ? 14 : 20 }]}>
      <View style={{ backgroundColor: '#D9F1F6', borderRadius: 18, padding: compactHeader ? 20 : 42, flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: compactHeader ? 20 : 42, overflow: 'hidden' }}>
        ...
        <View style={{ flex: 1, minWidth: compactHeader ? '100%' : 330, backgroundColor: COLORS.white, borderRadius: 16, minHeight: compactHeader ? 210 : 310, alignItems: 'center', justifyContent: 'center', padding: 22 }}>{isTcddHome ? <Image source={TCDD_LOGO} style={{ width: compactHeader ? 190 : 280, height: compactHeader ? 170 : 260, resizeMode: 'contain' }} /> : <View style={{ alignItems: 'center', gap: 12 }}><View style={{ width: compactHeader ? 130 : 190, height: compactHeader ? 130 : 190, borderRadius: 95, borderWidth: 5, borderColor: COLORS.red, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: COLORS.red, fontSize: compactHeader ? 32 : 46, fontWeight: '900' }}>T.C.</Text><Text style={{ color: COLORS.red, fontSize: compactHeader ? 10 : 13, fontWeight: '800', textAlign: 'center' }}>İÇİŞLERİ{`\n`}BAKANLIĞI</Text></View><Text style={{ color: COLORS.ink, fontWeight: '800' }}>2 unvan · sınav hazırlığı</Text></View>}</View>
      </View>
  ...
  function CurrentExamCards() { const institution = INSTITUTIONS[0]; const roles = institution.roles; const showInfo = (selectedRole: Role) => { setInfoRole(selectedRole); setScreen('role-info'); }; const cardsPerPage = compactHeader ? 1 : 4; const cardWidth = compactHeader ? 245 : Math.max(275, Math.floor((windowWidth - 108) / 4)); const pageCount = Math.ceil(roles.length / cardsPerPage); const moveCarousel = (direction: number) => setExamCarouselPage(currentPage => Math.max(0, Math.min(pageCount - 1, currentPage + direction))); const visibleRoles = compactHeader ? roles : roles.slice(examCarouselPage * cardsPerPage, (examCarouselPage + 1) * cardsPerPage); return <View style={{ padding: compactHeader ? 14 : 22, gap: 14, backgroundColor: COLORS.bg }}><View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}><Text style={styles.heading}>Güncel Sınavlar</Text>{compactHeader ? <Text style={{ color: COLORS.muted, fontSize: 12 }}>Kaydırarak incele →</Text> : <View style={{ flexDirection: 'row', gap: 8 }}><Pressable disabled={examCarouselPage === 0} onPress={() => moveCarousel(-1)} style={{ width: 38, height: 38, borderRadius: 19, borderWidth: 1, borderColor: examCarouselPage === 0 ? COLORS.line : COLORS.blue, alignItems: 'center', justifyContent: 'center', backgroundColor: examCarouselPage === 0 ? '#F4F7F9' : COLORS.white }}><Text style={{ color: examCarouselPage === 0 ? COLORS.muted : COLORS.blue, fontSize: 25, lineHeight: 27 }}>‹</Text></Pressable><Pressable disabled={examCarouselPage === pageCount - 1} onPress={() => moveCarousel(1)} style={{ width: 38, height: 38, borderRadius: 19, borderWidth: 1, borderColor: examCarouselPage === pageCount - 1 ? COLORS.line : COLORS.blue, alignItems: 'center', justifyContent: 'center', backgroundColor: examCarouselPage === pageCount - 1 ? '#F4F7F9' : COLORS.white }}><Text style={{ color: examCarouselPage === pageCount - 1 ? COLORS.muted : COLORS.blue, fontSize: 25, lineHeight: 27 }}>›</Text></Pressable></View>}</View>{compactHeader ? <ScrollView ref={examCarouselRef} horizontal showsHorizontalScrollIndicator={false}><View style={{ flexDirection: 'row', gap: 14 }}>{visibleRoles.map(item => <Pressable key={item.name} onPress={() => showInfo(item.name)} style={[styles.card, { width: cardWidth, minHeight: 270, alignItems: 'center', justifyContent: 'space-between' }]}><Image source={TCDD_LOGO} style={{ width: 142, height: 142, resizeMode: 'contain' }} /><Text style={{ textAlign: 'center', color: COLORS.ink, fontWeight: '800' }}>{item.name}</Text><Text style={{ textAlign: 'center', color: COLORS.muted, fontSize: 12 }}>{item.special}</Text><View style={styles.primaryButton}><Text style={styles.primaryText}>Konuları İncele</Text></View></Pressable>)}</View></ScrollView> : <View style={{ flexDirection: 'row', gap: 14 }}>{visibleRoles.map(item => <Pressable key={item.name} onPress={() => showInfo(item.name)} style={[styles.card, { width: cardWidth, minHeight: 270, alignItems: 'center', justifyContent: 'space-between' }]}><Image source={TCDD_LOGO} style={{ width: 142, height: 142, resizeMode: 'contain' }} /><Text style={{ textAlign: 'center', color: COLORS.ink, fontWeight: '800' }}>{item.name}</Text><Text style={{ textAlign: 'center', color: COLORS.muted, fontSize: 12 }}>{item.special}</Text><View style={styles.primaryButton}><Text style={styles.primaryText}>Konuları İncele</Text></View></Pressable>)}</View>}{!compactHeader && <View style={{ flexDirection: 'row', justifyContent: 'center', gap: 7 }}>{Array.from({ length: pageCount }).map((_, dotIndex) => <View key={dotIndex} style={{ width: dotIndex === examCarouselPage ? 20 : 8, height: 8, borderRadius: 4, backgroundColor: dotIndex === examCarouselPage ? COLORS.blue : '#DCE5EB' }} />)}</View>}</View>; }
  function CurrentExamCardsV2() {
    const cards = INSTITUTIONS.flatMap(institution => institution.roles.map(role => ({ institution, role })));
    const cardsPerPage = compactHeader ? 1 : 4;
    const cardWidth = compactHeader ? 245 : Math.max(275, Math.floor((windowWidth - 108) / 4));
    const pageCount = Math.ceil(cards.length / cardsPerPage);
    const visibleCards = compactHeader ? cards : cards.slice(examCarouselPage * cardsPerPage, (examCarouselPage + 1) * cardsPerPage);
    const move = (direction: number) => setExamCarouselPage(current => Math.max(0, Math.min(pageCount - 1, current + direction)));
    const openCard = (institution: Institution, role: Role) => { setSelectedInstitutionId(institution.id); setInfoRole(role); setScreen('role-info'); };
    return <View style={{ padding: compactHeader ? 14 : 22, gap: 14, backgroundColor: COLORS.bg }}><View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}><View><Text style={styles.heading}>Güncel Sınavlar</Text><Text style={styles.small}>Kurum ve unvanını seçerek konu kapsamını incele.</Text></View>{compactHeader ? <Text style={{ color: COLORS.muted, fontSize: 12 }}>Kaydırarak incele →</Text> : <View style={{ flexDirection: 'row', gap: 8 }}><Pressable disabled={examCarouselPage === 0} onPress={() => move(-1)} style={[styles.outlineButton, { opacity: examCarouselPage === 0 ? .45 : 1 }]}><Text style={styles.outlineText}>‹</Text></Pressable><Pressable disabled={examCarouselPage === pageCount - 1} onPress={() => move(1)} style={[styles.outlineButton, { opacity: examCarouselPage === pageCount - 1 ? .45 : 1 }]}><Text style={styles.outlineText}>›</Text></Pressable></View>}</View><ScrollView horizontal={compactHeader} showsHorizontalScrollIndicator={false}><View style={{ flexDirection: 'row', gap: 14 }}>{visibleCards.map(({ institution, role }) => <Pressable key={`${institution.id}-${role.name}`} onPress={() => openCard(institution, role.name)} style={[styles.card, { width: cardWidth, minHeight: 264, alignItems: 'center', justifyContent: 'space-between' }]}>{institution.logo ? <Image source={institution.logo} style={{ width: 118, height: 104, resizeMode: 'contain' }} /> : <View style={{ width: 104, height: 104, borderRadius: 52, borderWidth: 3, borderColor: COLORS.red, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: COLORS.red, fontWeight: '900', fontSize: 23 }}>T.C.</Text><Text style={{ color: COLORS.red, fontWeight: '900', fontSize: 8 }}>İÇİŞLERİ</Text></View>}<View style={{ alignItems: 'center', gap: 4 }}><Text style={{ color: institution.id === 'icisleri' ? COLORS.red : COLORS.blue, fontSize: 11, fontWeight: '900' }}>{institution.name}</Text><Text style={{ textAlign: 'center', color: COLORS.ink, fontWeight: '900' }}>{role.name}</Text><Text style={{ textAlign: 'center', color: COLORS.muted, fontSize: 12 }}>{role.special}</Text></View><View style={styles.primaryButton}><Text style={styles.primaryText}>Konuları İncele</Text></View></Pressable>)}</View></ScrollView>{!compactHeader && <View style={{ flexDirection: 'row', justifyContent: 'center', gap: 7 }}>{Array.from({ length: pageCount }).map((_, index) => <View key={index} style={{ width: index === examCarouselPage ? 20 : 8, height: 8, borderRadius: 4, backgroundColor: index === examCarouselPage ? COLORS.blue : '#DCE5EB' }} />)}</View>}</View>;
  }
  function RoleInfo() { const topics = topicsForRole(infoRole); return <ScrollView contentContainerStyle={styles.page}><Pressable onPress={() => setScreen('exams')} style={[styles.outlineButton, { alignSelf: 'flex-start' }]}><Text style={styles.outlineText}>← Sınavlara dön</Text></Pressable><View style={styles.hero}><Text style={styles.heroTitle}>{infoRole}</Text><Text style={styles.heroText}>Bu unvan için sınav kapsamındaki konu başlıklarını aşağıda inceleyebilirsin.</Text></View><View style={styles.card}><Text style={styles.cardTitle}>Sorumlu olunan konular</Text><Text style={styles.cardText}>Toplam {topics.length} konu bulunuyor.</Text><View style={{ gap: 9, marginTop: 6 }}>{topics.map((topic, index) => <View key={topic} style={{ flexDirection: 'row', gap: 10, alignItems: 'flex-start', borderBottomWidth: index === topics.length - 1 ? 0 : 1, borderBottomColor: COLORS.line, paddingBottom: 9 }}><View style={{ width: 26, height: 26, borderRadius: 13, backgroundColor: '#E8F0F6', alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: COLORS.blue, fontWeight: '900', fontSize: 12 }}>{index + 1}</Text></View><View style={{ flex: 1 }}><Text style={{ color: COLORS.ink, fontWeight: '700', lineHeight: 20 }}>{topic}</Text></View></View>)}</View></View><Pressable style={styles.primaryButton} onPress={() => user ? openStudy(infoRole) : (() => { setAuthMode('signup'); setScreen('auth'); })()}><Text style={styles.primaryText}>{user ? 'Bu unvan için çalışmaya başla' : 'Ücretsiz üyelikle çalışmaya başla'}</Text></Pressable></ScrollView>; }
  function openLegal(topic: LegalTopic) { setLegalTopic(topic); setScreen('legal'); }
  function Footer() { return <View style={{ backgroundColor: COLORS.navy, borderRadius: 12, padding: 20, gap: 18 }}><View style={{ gap: 5 }}><Text style={{ color: COLORS.white, fontSize: 20, fontWeight: '900' }}>TERFİ <Text style={{ color: '#FF8A8E' }}>ROTASI</Text></Text><Text style={{ color: '#C7D8E5', fontSize: 12, lineHeight: 18 }}>Görevde yükselme sınavlarına yönelik dijital hazırlık platformu.</Text></View><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 22 }}><View style={{ gap: 7, minWidth: 140 }}><Text style={{ color: '#FFB0B3', fontWeight: '800', fontSize: 12 }}>HIZLI BAĞLANTILAR</Text><Pressable onPress={() => setScreen('exams')}><Text style={{ color: COLORS.white }}>Sınavlar</Text></Pressable><Pressable onPress={() => setScreen('membership')}><Text style={{ color: COLORS.white }}>Üyelik paketleri</Text></Pressable><Pressable onPress={() => setScreen('auth')}><Text style={{ color: COLORS.white }}>Hesabım</Text></Pressable></View><View style={{ gap: 8, minWidth: 210 }}><Text style={{ color: '#FFB0B3', fontWeight: '800', fontSize: 12 }}>YASAL BİLGİLER</Text><Pressable onPress={() => openLegal('membership')}><Text style={{ color: COLORS.white }}>Üyelik Sözleşmesi</Text></Pressable><Pressable onPress={() => openLegal('privacy')}><Text style={{ color: COLORS.white }}>Gizlilik Politikası</Text></Pressable><Pressable onPress={() => openLegal('kvkk')}><Text style={{ color: COLORS.white }}>KVKK Aydınlatma Metni</Text></Pressable><Pressable onPress={() => openLegal('cookies')}><Text style={{ color: COLORS.white }}>Çerez Politikası</Text></Pressable></View></View><View style={{ height: 1, backgroundColor: '#FFFFFF2A' }} /><Text style={{ color: '#B8CBD9', fontSize: 11 }}>© 2026 Terfi Rotası · Yasal metinler yayımlanmadan önce veri sorumlusu ve iletişim bilgileriyle tamamlanmalıdır.</Text></View>; }
  function Legal() {
    const content: Record<LegalTopic, { title: string; intro: string; sections: [string, string][] }> = {
      membership: { title: 'Üyelik Sözleşmesi', intro: 'Bu metin, Terfi Rotası hesabının kullanımı için temel kuralları açıklar.', sections: [['Hizmetin kapsamı', 'Terfi Rotası; sınav konuları, soru çözümü, deneme ve analiz özellikleri sunar. Ücretsiz üyelikteki kullanım hakları platformda ayrıca belirtilir.'], ['Kullanıcı sorumluluğu', 'Hesap bilgilerini korumak, şifreyi üçüncü kişilerle paylaşmamak ve hizmeti hukuka uygun kullanmak kullanıcının sorumluluğundadır.'], ['İçerik kullanımı', 'Soru ve eğitim içerikleri yalnızca kişisel hazırlık amacıyla kullanılabilir; çoğaltılamaz, yayımlanamaz veya ticari amaçla paylaşılamaz.'], ['İletişim', 'Üyelik, içerik veya veri taleplerin için yayın sahibinin iletişim bilgileri bu alana eklenecektir.']] },
      privacy: { title: 'Gizlilik Politikası', intro: 'Terfi Rotası, hesabın oluşturulması ve hizmetin sunulması için gerekli bilgileri korumayı hedefler.', sections: [['Toplanan bilgiler', 'Ad soyad, e-posta, telefon, il, seçilen unvan, üyelik ve çalışma verileri işlenebilir.'], ['Kullanım amaçları', 'Bilgiler; hesabın yönetilmesi, soru çözüm haklarının uygulanması, destek sağlanması ve platform güvenliğinin korunması için kullanılır.'], ['Hizmet sağlayıcılar', 'Kimlik doğrulama ve veri depolama hizmetleri Supabase altyapısı üzerinden; yayın hizmeti Vercel altyapısı üzerinden sağlanmaktadır.'], ['Güvenlik', 'Erişim yetkileri, oturum yönetimi ve güvenli bağlantı gibi teknik tedbirler uygulanır. Hiçbir çevrim içi sistem için mutlak güvenlik garantisi verilemez.']] },
      kvkk: { title: 'KVKK Aydınlatma Metni', intro: 'Kişisel verilerinin işlenmesine ilişkin temel bilgilendirme metnidir.', sections: [['Veri sorumlusu', 'Bu bölüm, yayına geçmeden önce işletme sahibi/şirket unvanı, MERSİS veya vergi bilgisi varsa bunlar ve açık iletişim adresi ile tamamlanmalıdır.'], ['İşleme amacı ve hukuki sebep', 'Kayıt bilgileri üyelik hesabının kurulması, hizmetin sunulması, güvenlik ve yasal yükümlülüklerin yerine getirilmesi amaçlarıyla işlenir.'], ['Aktarım', 'Teknik hizmet sağlayıcılar, yasal zorunluluklar veya yetkili makamlar dışında veriler paylaşılmaz. Yurt dışı aktarım değerlendirmesi kullanılan hizmetlerin güncel ayarlarına göre ayrıca yapılmalıdır.'], ['Hakların', 'KVKK kapsamındaki bilgi talebi, düzeltme ve diğer başvuru hakların için veri sorumlusunun ilan edilecek başvuru kanalını kullanabilirsin.']] },
      cookies: { title: 'Çerez Politikası', intro: 'Çerezler, sitenin çalışması ve oturum yönetimi için sınırlı biçimde kullanılabilir.', sections: [['Zorunlu çerezler', 'Oturumun sürdürülmesi, güvenlik ve temel site işlevleri için gerekli veriler kullanılabilir.'], ['Tercih çerezleri', '“Beni Hatırla” seçeneği kullanıldığında e-posta adresi cihazında hatırlanabilir. Bu tercih profil veya tarayıcı ayarlarından kaldırılabilir.'], ['Üçüncü taraflar', 'Altyapı sağlayıcılarının kendi teknik çerezleri olabilir. Reklam veya pazarlama çerezi eklenirse bu politika ve izin mekanizması ayrıca güncellenecektir.'], ['Tercihlerin', 'Tarayıcı ayarlarından çerezleri silebilir veya engelleyebilirsin; bu durumda bazı özellikler çalışmayabilir.']] }
    };
    const page = content[legalTopic];
    return <ScrollView contentContainerStyle={[styles.page, { maxWidth: 940, width: '100%', alignSelf: 'center' }]}><Pressable style={[styles.outlineButton, { alignSelf: 'flex-start' }]} onPress={() => setScreen('home')}><Text style={styles.outlineText}>← Ana sayfaya dön</Text></Pressable><View style={styles.hero}><Text style={styles.heroTitle}>{page.title}</Text><Text style={styles.heroText}>Son güncelleme: 16 Ağustos 2026</Text></View><View style={styles.card}><Text style={[styles.cardText, { lineHeight: 21 }]}>{page.intro}</Text>{page.sections.map(([title, text]) => <View key={title} style={{ gap: 5, paddingTop: 8 }}><Text style={{ color: COLORS.ink, fontSize: 16, fontWeight: '900' }}>{title}</Text><Text style={[styles.cardText, { lineHeight: 21 }]}>{text}</Text></View>)}<View style={{ marginTop: 8, backgroundColor: '#FFF7E4', borderLeftWidth: 4, borderLeftColor: COLORS.gold, padding: 12, borderRadius: 8 }}><Text style={{ color: COLORS.gold, fontWeight: '800', lineHeight: 19 }}>Not: Bu taslak, platform bilgilerine göre son hâline getirilmeli ve yayın öncesi hukuk uzmanı tarafından gözden geçirilmelidir.</Text></View></View><Footer /></ScrollView>;
  }
  ...
  function ExamsAccordion() {
    ...
    if (!selectedInstitution) return <ScrollView contentContainerStyle={[styles.page, { maxWidth: 1180, width: '100%', alignSelf: 'center' }]}><View style={{ gap: 5 }}><Text style={{ color: COLORS.blue, fontSize: 12, fontWeight: '900', letterSpacing: .6 }}>SINAV MERKEZİ</Text><Text style={[styles.heading, { fontSize: compactHeader ? 26 : 30, marginTop: 0 }]}>Hangi sınava hazırlanıyorsun?</Text><Text style={styles.cardText}>Önce kurumunu seç; ardından o kuruma ait unvanlar ve konular açılır.</Text></View><View style={{ backgroundColor: COLORS.white, borderWidth: 1, borderColor: institutionOpen ? COLORS.blue : COLORS.line, borderRadius: 12, overflow: 'hidden' }}><Pressable onPress={() => setInstitutionOpen(value => !value)} style={{ padding: compactHeader ? 14 : 17, flexDirection: 'row', alignItems: 'center', gap: 12 }}><View style={{ width: 48, height: 42, borderRadius: 9, backgroundColor: '#EAF5FA', alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: COLORS.blue, fontWeight: '900', fontSize: 20 }}>⌄</Text></View><View style={{ flex: 1, gap: 3 }}><Text style={{ color: COLORS.muted, fontSize: 11, fontWeight: '800' }}>SEÇİLİ KURUM</Text><Text style={{ color: COLORS.muted, fontWeight: '800' }}>Kurum seç</Text></View><Text style={{ color: COLORS.blue, fontSize: 22, fontWeight: '900' }}>{institutionOpen ? '⌃' : '⌄'}</Text></Pressable>{institutionOpen && <View style={{ borderTopWidth: 1, borderTopColor: COLORS.line }}>{INSTITUTIONS.map(item => <Pressable key={item.id} onPress={() => { setSelectedInstitutionId(item.id); setExpandedExamRole(null); setInstitutionOpen(false); }} style={{ flexDirection: 'row', alignItems: 'center', gap: 11, paddingHorizontal: 14, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: COLORS.line }}>{item.logo ? <Image source={item.logo} style={{ width: 38, height: 32, resizeMode: 'contain' }} /> : <View style={{ width: 38, height: 32, borderRadius: 7, backgroundColor: '#FFF0F1', alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: COLORS.red, fontWeight: '900', fontSize: 11 }}>{item.mark}</Text></View>}<View style={{ flex: 1 }}><Text style={{ color: COLORS.ink, fontWeight: '900' }}>{item.name}</Text><Text style={{ color: COLORS.muted, fontSize: 12, marginTop: 2 }}>{item.detail} · {item.roles.length} unvan</Text></View><Text style={{ color: COLORS.blue, fontWeight: '900' }}>Seç</Text></Pressable>)}</View>}</View><View style={[styles.card, { alignItems: 'center', paddingVertical: compactHeader ? 25 : 40 }]}><Text style={{ color: COLORS.ink, fontWeight: '900', fontSize: 18 }}>Kurum seçimini bekliyoruz</Text><Text style={[styles.cardText, { textAlign: 'center' }]}>Kurum seçildiğinde ilgili unvanlar burada listelenecek.</Text></View><Footer /></ScrollView>;
    const institution = selectedInstitution;
    const filteredRoles = institution.roles.filter(item => `${item.name} ${item.special}`.toLocaleLowerCase('tr-TR').includes(examSearch.trim().toLocaleLowerCase('tr-TR')));
    return <ScrollView contentContainerStyle={[styles.page, { maxWidth: 1180, width: '100%', alignSelf: 'center' }]}>
      ...
      <View style={{ gap: 8 }}><Pressable onPress={() => setInstitutionOpen(value => !value)} style={{ backgroundColor: COLORS.white, borderWidth: 1, borderColor: institutionOpen ? COLORS.blue : COLORS.line, borderRadius: 12, padding: compactHeader ? 14 : 17, flexDirection: 'row', alignItems: 'center', gap: 12 }}>{institution.logo ? <Image source={institution.logo} style={{ width: compactHeader ? 46 : 54, height: compactHeader ? 40 : 46, resizeMode: 'contain' }} /> : <View style={{ width: 46, height: 40, borderRadius: 8, backgroundColor: '#FFF0F1', alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: COLORS.red, fontWeight: '900' }}>{institution.mark}</Text></View>}<View style={{ flex: 1, gap: 3 }}><Text style={{ color: COLORS.muted, fontSize: 11, fontWeight: '800' }}>SEÇİLİ KURUM</Text><Text style={{ color: COLORS.ink, fontWeight: '900', fontSize: compactHeader ? 15 : 16 }}>{institution.name}</Text><Text style={{ color: COLORS.muted, fontSize: 12 }}>{institution.detail} · {institution.roles.length} unvan</Text></View><Text style={{ color: COLORS.blue, fontSize: 22, fontWeight: '900' }}>{institutionOpen ? '⌃' : '⌄'}</Text></Pressable>{institutionOpen && <View style={{ borderWidth: 1, borderColor: COLORS.line, borderRadius: 10, overflow: 'hidden', backgroundColor: COLORS.white }}>{INSTITUTIONS.map(item => <Pressable key={item.id} onPress={() => { setSelectedInstitutionId(item.id); setExpandedExamRole(null); setInstitutionOpen(false); }} style={{ flexDirection: 'row', alignItems: 'center', gap: 11, paddingHorizontal: 14, paddingVertical: 13, backgroundColor: selectedInstitutionId === item.id ? '#EAF5FA' : COLORS.white }}>{item.logo ? <Image source={item.logo} style={{ width: 34, height: 30, resizeMode: 'contain' }} /> : <View style={{ width: 34, height: 30, borderRadius: 7, backgroundColor: '#FFF0F1', alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: COLORS.red, fontWeight: '900', fontSize: 11 }}>{item.mark}</Text></View>}<Text style={{ flex: 1, color: selectedInstitutionId === item.id ? COLORS.blue : COLORS.ink, fontWeight: '900' }}>{item.name}</Text>{selectedInstitutionId === item.id && <Text style={{ color: COLORS.blue, fontWeight: '900' }}>✓</Text>}</Pressable>)}</View>}</View>
      <View style={{ gap: 5, marginTop: 4 }}><Text style={styles.heading}>{examSearch.trim() ? `“${examSearch}” için sonuçlar` : 'Unvanını seç'}</Text><Text style={styles.cardText}>{examSearch.trim() ? `${filteredRoles.length} unvan bulundu.` : 'Unvana dokun; konular hemen kartın altında açılsın.'}</Text></View>
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>{filteredRoles.map((item, index) => { const isExpanded = expandedExamRole === item.name; const topics = item.topics; return <View key={item.name} style={{ width: compactHeader ? '100%' : '48.9%', minWidth: compactHeader ? 0 : 330, gap: 0 }}><Pressable onPress={() => setExpandedExamRole(current => current === item.name ? null : item.name)} style={{ backgroundColor: isExpanded ? '#F2F8FB' : COLORS.white, borderWidth: isExpanded ? 2 : 1, borderColor: isExpanded ? COLORS.blue : COLORS.line, borderRadius: 10, padding: compactHeader ? 14 : 16, flexDirection: 'row', alignItems: 'center', gap: 12 }}><View style={{ width: 30, height: 30, borderRadius: 15, backgroundColor: index % 2 ? '#EAF5FA' : '#FFF0F1', alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: index % 2 ? COLORS.blue : COLORS.red, fontWeight: '900', fontSize: 12 }}>{String(index + 1).padStart(2, '0')}</Text></View><View style={{ flex: 1, gap: 3 }}><Text style={{ color: COLORS.ink, fontSize: compactHeader ? 15 : 16, fontWeight: '900', lineHeight: 21 }}>{item.name}</Text><Text numberOfLines={1} style={{ color: COLORS.muted, fontSize: 12 }}>{item.special}</Text></View><Text style={{ color: COLORS.blue, fontSize: 22, fontWeight: '900' }}>{isExpanded ? '⌃' : '⌄'}</Text></Pressable>{isExpanded && <View style={{ borderWidth: 1, borderTopWidth: 0, borderColor: COLORS.blue, borderBottomLeftRadius: 10, borderBottomRightRadius: 10, backgroundColor: COLORS.white, padding: 13, gap: 8 }}><Text style={{ color: COLORS.blue, fontSize: 11, fontWeight: '900' }}>SORUMLU OLUNAN KONULAR · {topics.length}</Text>{topics.map((topic, topicIndex) => <View key={topic} style={{ flexDirection: 'row', gap: 8, alignItems: 'flex-start', borderTopWidth: topicIndex ? 1 : 0, borderTopColor: COLORS.line, paddingTop: topicIndex ? 7 : 0 }}><Text style={{ color: COLORS.blue, fontWeight: '900', fontSize: 12 }}>{topicIndex + 1}.</Text><Text style={{ flex: 1, color: COLORS.ink, fontSize: 13, lineHeight: 18 }}>{topic}</Text></View>)}<Pressable style={[styles.primaryButton, { marginTop: 4 }]} onPress={() => user ? openStudy(item.name) : (() => { setAuthInstitutionId(institution.id); setAuthRole(item.name); setAuthRoleSelected(true); setAuthMode('signup'); setScreen('auth'); })()}><Text style={styles.primaryText}>{user ? 'ÇALIŞMAYA BAŞLA' : 'ÜCRETSİZ BAŞLA'}</Text></Pressable></View>}</View>; })}{filteredRoles.length === 0 && <View style={styles.card}><Text style={styles.cardText}>Aramana uygun unvan bulunamadı.</Text></View>}</View>
      <Footer />
    </ScrollView>;
  }
  function Quiz() {
  ...
  function StudySetup() {
    const availableTopics = topicsForRole(studyRole);
  ...
  function ResetPassword() { return <ScrollView contentContainerStyle={styles.page}><View style={{ width: '100%', maxWidth: 560, alignSelf: 'center', gap: 14 }}><View style={styles.hero}><Text style={styles.heroTitle}>Yeni şifreni belirle</Text><Text style={styles.heroText}>Hesabın için güvenli bir yeni şifre oluştur.</Text></View><View style={styles.card}><Text style={styles.inputLabel}>Yeni şifre</Text><TextInput value={newPassword} onChangeText={setNewPassword} placeholder="En az 6 karakter" style={styles.input} secureTextEntry autoCapitalize="none" autoComplete="new-password" /><Text style={styles.inputLabel}>Yeni şifre tekrar</Text><TextInput value={newPasswordConfirm} onChangeText={setNewPasswordConfirm} placeholder="Şifreni yeniden yaz" style={styles.input} secureTextEntry autoCapitalize="none" autoComplete="new-password" />{resetFeedback && <View style={{ backgroundColor: resetFeedback.type === 'success' ? '#EAF7EF' : '#FFF0F1', borderLeftWidth: 4, borderLeftColor: resetFeedback.type === 'success' ? COLORS.green : COLORS.red, borderRadius: 7, padding: 11 }}><Text style={{ color: resetFeedback.type === 'success' ? COLORS.green : '#B4232C', fontWeight: '700', lineHeight: 19 }}>{resetFeedback.text}</Text></View>}<Pressable disabled={resetBusy} style={[styles.primaryButton, resetBusy && { opacity: .65 }]} onPress={completePasswordReset}><Text style={styles.primaryText}>{resetBusy ? 'ŞİFREN YENİLENİYOR...' : 'ŞİFREMİ YENİLE'}</Text></Pressable><Pressable onPress={() => { setAuthMode('login'); setScreen('auth'); }}><Text style={[styles.outlineText, { textAlign: 'center' }]}>Giriş ekranına dön</Text></Pressable></View></View></ScrollView>; }
  function Auth() {
    ...
    return <ScrollView contentContainerStyle={styles.page}><View style={{ flexDirection: 'row', flexWrap: 'wrap', borderWidth: 1, borderColor: COLORS.line, borderRadius: 16, overflow: 'hidden', backgroundColor: COLORS.white, shadowColor: '#15354B', shadowOpacity: 0.06, shadowRadius: 18, elevation: 2 }}><View style={{ flex: compactHeader ? undefined : 1, width: compactHeader ? '100%' : undefined, minWidth: compactHeader ? 0 : 340, padding: compactHeader ? 14 : 22, gap: 14 }}>
      ...
      <View style={styles.card}>
        {register && <><Text style={styles.inputLabel}>Ad Soyad</Text><TextInput value={authName} onChangeText={setAuthName} placeholder="Adın ve soyadın" style={[styles.input, { backgroundColor: '#F3F7FA' }]} autoCapitalize="words" /><Text style={styles.inputLabel}>Telefon</Text><TextInput value={authPhone} onChangeText={setAuthPhone} placeholder="05XX XXX XX XX" style={[styles.input, { backgroundColor: '#F3F7FA' }]} keyboardType="phone-pad" /></>}
        <Text style={styles.inputLabel}>E-posta adresin</Text><TextInput key={`email-${register ? 'signup' : 'login'}`} value={authEmail} onChangeText={setAuthEmail} placeholder="ornek@email.com" style={[styles.input, register && { backgroundColor: '#F3F7FA' }]} autoCapitalize="none" keyboardType="email-address" autoComplete={register ? 'off' : 'email'} />
        <Text style={styles.inputLabel}>Şifre</Text><TextInput key={`password-${register ? 'signup' : 'login'}`} value={authPassword} onChangeText={setAuthPassword} placeholder="En az 6 karakter" style={[styles.input, register && { backgroundColor: '#F3F7FA' }]} secureTextEntry autoCapitalize="none" autoComplete={register ? 'new-password' : 'current-password'} />
        {!register && <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 2 }}><Pressable onPress={() => setAuthRemember(value => !value)} style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}><View style={{ width: 20, height: 20, borderRadius: 5, borderWidth: 1, borderColor: authRemember ? COLORS.blue : COLORS.line, backgroundColor: authRemember ? COLORS.blue : COLORS.white, alignItems: 'center', justifyContent: 'center' }}>{authRemember && <Text style={{ color: COLORS.white, fontWeight: '900' }}>✓</Text>}</View><Text style={{ color: COLORS.muted, fontSize: 13 }}>Beni Hatırla</Text></Pressable><Pressable onPress={async () => { if (!authEmail.includes('@')) { setAuthFeedback({ type: 'error', text: 'Önce e-posta adresini yazmalısın.' }); return; } try { await sendPasswordResetEmail(authEmail.trim().toLowerCase(), 'https://terfirotasi.vercel.app'); setAuthFeedback({ type: 'success', text: 'Şifre sıfırlama bağlantısı e-posta adresine gönderildi. Bağlantıya tıkladığında yeni şifre ekranı açılacak.' }); } catch (error) { setAuthFeedback({ type: 'error', text: error instanceof Error ? error.message : 'Şifre sıfırlama bağlantısı gönderilemedi.' }); } }}><Text style={{ color: COLORS.blue, fontWeight: '800', fontSize: 13 }}>Şifremi Unuttum</Text></Pressable></View>}
        {register && <>
          <Text style={styles.inputLabel}>Şifre Tekrar</Text><TextInput key="password-confirm-signup" value={authPasswordConfirm} onChangeText={setAuthPasswordConfirm} placeholder="Şifreni yeniden yaz" style={[styles.input, { backgroundColor: '#F3F7FA' }]} secureTextEntry autoCapitalize="none" autoComplete="new-password" />
          <Text style={styles.inputLabel}>Kurumun</Text><Pressable onPress={() => { setAuthInstitutionOpen(value => !value); setAuthRoleOpen(false); }} style={[styles.input, { backgroundColor: '#F3F7FA', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }]}><Text style={{ color: authInstitutionId ? COLORS.ink : COLORS.muted }}>{INSTITUTIONS.find(item => item.id === authInstitutionId)?.name || 'Kurum seç'}</Text><Text style={{ color: COLORS.blue, fontSize: 16, fontWeight: '900' }}>{authInstitutionOpen ? '⌃' : '⌄'}</Text></Pressable>{authInstitutionOpen && <View style={{ borderWidth: 1, borderColor: COLORS.line, borderRadius: 8, backgroundColor: COLORS.white, overflow: 'hidden' }}>{INSTITUTIONS.map(item => <Pressable key={item.id} onPress={() => { setAuthInstitutionId(item.id); setAuthRole(item.roles[0].name); setAuthRoleSelected(false); setAuthInstitutionOpen(false); }} style={{ paddingHorizontal: 15, paddingVertical: 12, backgroundColor: authInstitutionId === item.id ? '#EAF5FA' : COLORS.white }}><Text style={{ color: authInstitutionId === item.id ? COLORS.blue : COLORS.ink, fontWeight: authInstitutionId === item.id ? '800' : '500' }}>{item.name}</Text><Text style={{ color: COLORS.muted, fontSize: 12, marginTop: 2 }}>{item.detail}</Text></Pressable>)}</View>}
          <Text style={styles.inputLabel}>Unvanın</Text><Pressable onPress={() => { if (!authInstitutionId) { setAuthFeedback({ type: 'error', text: 'Önce kurumunu seçmelisin.' }); return; } setAuthRoleOpen(value => !value); setAuthInstitutionOpen(false); }} style={[styles.input, { backgroundColor: '#F3F7FA', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }]}><Text style={{ color: authRoleSelected ? COLORS.ink : COLORS.muted }}>{authRoleSelected ? authRole : 'Unvan seç'}</Text><Text style={{ color: COLORS.blue, fontSize: 16, fontWeight: '900' }}>{authRoleOpen ? '⌃' : '⌄'}</Text></Pressable>{authRoleOpen && <View style={{ maxHeight: 260, borderWidth: 1, borderColor: COLORS.line, borderRadius: 8, backgroundColor: COLORS.white, overflow: 'hidden' }}><ScrollView nestedScrollEnabled>{(INSTITUTIONS.find(item => item.id === authInstitutionId)?.roles ?? []).map(item => <Pressable key={item.name} onPress={() => { setAuthRole(item.name); setAuthRoleSelected(true); setAuthRoleOpen(false); }} style={{ borderBottomWidth: 1, borderBottomColor: COLORS.line, paddingHorizontal: 15, paddingVertical: 11, backgroundColor: authRoleSelected && authRole === item.name ? '#EAF5FA' : COLORS.white }}><Text style={{ color: authRoleSelected && authRole === item.name ? COLORS.blue : COLORS.ink, fontWeight: authRoleSelected && authRole === item.name ? '800' : '500' }}>{item.name}</Text></Pressable>)}</ScrollView></View>}
          <Text style={styles.inputLabel}>İlin</Text><Pressable onPress={() => setAuthCityOpen(value => !value)} style={[styles.input, { backgroundColor: '#F3F7FA', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }]}><Text style={{ color: authCity ? COLORS.ink : COLORS.muted }}>{authCity || 'İl seç'}</Text><Text style={{ color: COLORS.blue, fontSize: 16, fontWeight: '900' }}>{authCityOpen ? '⌃' : '⌄'}</Text></Pressable>{authCityOpen && <View style={{ maxHeight: 300, borderWidth: 1, borderColor: COLORS.line, borderRadius: 8, backgroundColor: COLORS.white, overflow: 'hidden' }}><ScrollView nestedScrollEnabled showsVerticalScrollIndicator contentContainerStyle={{ paddingVertical: 4 }}><View style={{ paddingHorizontal: 15, paddingVertical: 10, backgroundColor: COLORS.blue }}><Text style={{ color: COLORS.white, fontWeight: '800' }}>Bir İl Seçin</Text></View>{CITIES.map(city => <Pressable key={city} onPress={() => { setAuthCity(city); setAuthCityOpen(false); }} style={{ borderBottomWidth: 1, borderBottomColor: COLORS.line, paddingHorizontal: 15, paddingVertical: 11, backgroundColor: authCity === city ? '#EAF5FA' : COLORS.white }}><Text style={{ color: authCity === city ? COLORS.blue : COLORS.ink, fontSize: 14, fontWeight: authCity === city ? '800' : '500' }}>{city}</Text></Pressable>)}</ScrollView></View>}
          <Text style={styles.inputLabel}>Güvenlik kodu</Text><View style={{ flexDirection: 'row', gap: 9, alignItems: 'stretch' }}><View style={{ minWidth: 104, borderRadius: 8, backgroundColor: '#FFF1C9', borderWidth: 1, borderColor: '#EDC55B', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 12 }}><Text style={{ color: '#76510A', fontSize: 18, fontWeight: '900', letterSpacing: 3 }}>{securityCode}</Text></View><TextInput value={securityEntry} onChangeText={setSecurityEntry} placeholder="Kodu yaz" style={[styles.input, { backgroundColor: '#F3F7FA', flex: 1 }]} autoCapitalize="characters" /><Pressable onPress={() => { setSecurityCode(makeSecurityCode()); setSecurityEntry(''); }} style={{ borderWidth: 1, borderColor: COLORS.line, borderRadius: 8, width: 46, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: COLORS.blue, fontSize: 20 }}>↻</Text></Pressable></View><Text style={{ color: COLORS.muted, fontSize: 11 }}>Kayıt işlemini tamamlamak için soldaki kodu yaz.</Text>
          <Pressable style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 9, marginTop: 4 }} onPress={() => setAuthAccepted(value => !value)}><View style={{ width: 20, height: 20, borderRadius: 5, borderWidth: 1, borderColor: authAccepted ? COLORS.blue : COLORS.line, backgroundColor: authAccepted ? COLORS.blue : COLORS.white, alignItems: 'center', justifyContent: 'center', marginTop: 1 }}>{authAccepted && <Text style={{ color: COLORS.white, fontWeight: '800' }}>✓</Text>}</View><Text style={{ flex: 1, color: COLORS.muted, fontSize: 12, lineHeight: 17 }}>Üyelik Sözleşmesi’ni ve Gizlilik / KVKK Aydınlatma Metni’ni okudum; üyelik hesabım için gerekli bilgilerimin işlenmesi hakkında bilgilendirildim.</Text></Pressable><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12, marginTop: -3 }}><Pressable onPress={() => setLegalModalTopic('membership')}><Text style={{ color: COLORS.blue, fontSize: 12, fontWeight: '800' }}>Üyelik Sözleşmesi</Text></Pressable><Pressable onPress={() => setLegalModalTopic('privacy')}><Text style={{ color: COLORS.blue, fontSize: 12, fontWeight: '800' }}>Gizlilik Politikası</Text></Pressable><Pressable onPress={() => setLegalModalTopic('kvkk')}><Text style={{ color: COLORS.blue, fontSize: 12, fontWeight: '800' }}>KVKK Metni</Text></Pressable></View>
        </>}
  ...
  function Admin() {
    ...
    return <><ScrollView contentContainerStyle={[styles.page, { maxWidth: 1280, width: '100%', alignSelf: 'center' }]}>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.chipWrap}>{INSTITUTIONS.flatMap(institution => institution.roles).map(item => <Pressable key={item.name} onPress={() => { setAdminRole(item.name); setAdminTopic(item.topics[0]); }} style={[styles.chip, adminRole === item.name && styles.chipActive]}><Text style={[styles.chipText, adminRole === item.name && styles.chipTextActive]}>{item.name}</Text></Pressable>)}</View>
