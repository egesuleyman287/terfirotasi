...
export default function App() {
  ...
  function Home() { const memberRole = (user?.role as Role) ?? ROLES[0].name; return <ScrollView contentContainerStyle={styles.page}><View style={{ backgroundColor: COLORS.navy, borderRadius: 12, padding: compactHeader ? 22 : 30, gap: 12 }}><Text style={{ color: '#A9DAF0', fontWeight: '900', fontSize: 12, letterSpacing: .6 }}>TCDD TAŞIMACILIK A.Ş. · SINAV HAZIRLIK PLATFORMU</Text><Text style={{ color: COLORS.white, fontSize: compactHeader ? 29 : 34, fontWeight: '900', lineHeight: compactHeader ? 37 : 42 }}>Kariyerinde bir sonraki adıma hazır mısın?</Text><Text style={{ color: '#E7F4FA', fontSize: 16, lineHeight: 23 }}>Terfi Rotası ile eksiklerini gör, yolunu çiz. Unvanına özel konu çalışmaları, soru bankası ve sınav analizi tek yerde.</Text><Pressable onPress={() => user ? openStudy(memberRole) : (() => { setAuthMode('signup'); setAuthFeedback(null); setScreen('auth'); })()} style={{ alignSelf: compactHeader ? 'stretch' : 'flex-start', backgroundColor: COLORS.red, paddingHorizontal: 22, paddingVertical: 14, borderRadius: 8, marginTop: 8, alignItems: 'center' }}><Text style={{ color: COLORS.white, fontSize: 15, fontWeight: '900' }}>{user ? 'HEMEN BAŞLA' : 'ROTANI OLUŞTUR'}</Text></Pressable></View><View style={{ flexDirection: 'row', gap: 9 }}><View style={styles.stat}><Text style={styles.statValue}>9</Text><Text style={styles.small}>Unvan</Text></View><View style={styles.stat}><Text style={styles.statValue}>11</Text><Text style={styles.small}>Sınav konusu</Text></View><View style={styles.stat}><Text style={styles.statValue}>{published}+</Text><Text style={styles.small}>Mevcut soru</Text></View></View><CurrentExamCards /><Footer /></ScrollView>; }
  function CurrentExamCards() { const showInfo = (selectedRole: Role) => { setInfoRole(selectedRole); setScreen('role-info'); }; const cardWidth = compactHeader ? 245 : Math.max(275, Math.floor((windowWidth - 108) / 4)); return <View style={{ padding: compactHeader ? 14 : 22, gap: 14, backgroundColor: COLORS.bg }}><View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}><Text style={styles.heading}>Güncel Sınavlar</Text><Text style={{ color: COLORS.muted, fontSize: 12 }}>{compactHeader ? 'Kaydırarak incele →' : 'Sağa kaydırarak diğer unvanları incele →'}</Text></View><ScrollView horizontal showsHorizontalScrollIndicator={!compactHeader} contentContainerStyle={{ paddingBottom: compactHeader ? 0 : 10 }}><View style={{ flexDirection: 'row', gap: 14 }}>{ROLES.map(item => <Pressable key={item.name} onPress={() => showInfo(item.name)} style={[styles.card, { width: cardWidth, minHeight: 270, alignItems: 'center', justifyContent: 'space-between' }]}><Image source={TCDD_LOGO} style={{ width: 142, height: 142, resizeMode: 'contain' }} /><Text style={{ textAlign: 'center', color: COLORS.ink, fontWeight: '800' }}>{item.name}</Text><Text style={{ textAlign: 'center', color: COLORS.muted, fontSize: 12 }}>{item.special}</Text><View style={styles.primaryButton}><Text style={styles.primaryText}>Konuları İncele</Text></View></Pressable>)}</View></ScrollView></View>; }
  ...
  function Exams() { return <ScrollView contentContainerStyle={[styles.page, { maxWidth: 1180, width: '100%', alignSelf: 'center' }]}><View style={{ backgroundColor: COLORS.navy, borderRadius: 14, padding: compactHeader ? 18 : 26, flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: 18 }}><View style={{ flex: 1, minWidth: 220, gap: 6 }}><Text style={{ color: '#A9DAF0', fontSize: 12, fontWeight: '900', letterSpacing: .7 }}>TCDD TAŞIMACILIK A.Ş.</Text><Text style={{ color: COLORS.white, fontSize: compactHeader ? 25 : 30, fontWeight: '900' }}>Görevde yükselme sınavları</Text><Text style={{ color: '#DDEFFC', lineHeight: 20 }}>Unvanını seçerek sınav kapsamını ve sorumlu olduğun konu başlıklarını incele.</Text></View><Image source={TCDD_LOGO} style={{ width: compactHeader ? 84 : 108, height: compactHeader ? 72 : 92, resizeMode: 'contain' }} /></View><View style={{ gap: 5, marginTop: 4 }}><Text style={styles.heading}>Sınav kataloğu</Text><Text style={styles.cardText}>Aşağıdaki unvanlardan birini seçerek ayrıntılı konu kapsamını görebilirsin.</Text></View><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12 }}>{ROLES.map((item, index) => <Pressable key={item.name} onPress={() => { setInfoRole(item.name); setScreen('role-info'); }} style={[styles.card, { width: compactHeader ? '100%' : '48.9%', minWidth: compactHeader ? 0 : 330, minHeight: compactHeader ? 136 : 146, padding: compactHeader ? 15 : 18, justifyContent: 'space-between', borderLeftWidth: 4, borderLeftColor: index % 3 === 0 ? COLORS.red : COLORS.blue }]}><View style={{ gap: 5 }}><Text style={{ color: COLORS.ink, fontSize: compactHeader ? 16 : 17, fontWeight: '900', lineHeight: 22 }}>{item.name}</Text><Text style={{ color: COLORS.muted, fontSize: 12, lineHeight: 17 }}>Unvana özel konu: {item.special}</Text></View><View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}><Text style={{ color: COLORS.blue, fontSize: 12, fontWeight: '900' }}>11 konu · Sınav kapsamı</Text><Text style={{ color: COLORS.blue, fontSize: 19, fontWeight: '900' }}>›</Text></View></Pressable>)}</View><Footer /></ScrollView>; }
  function Quiz() {
  ...
  return <SafeAreaView style={styles.safe}><StatusBar style="light" />{compactHeader ? MobileHeader : Header}{screen === 'home' && Home()}{screen === 'exams' && Exams()}{screen === 'role-info' && RoleInfo()}{screen === 'study' && StudySetup()}{screen === 'quiz' && Quiz()}{screen === 'result' && Result()}{screen === 'membership' && Membership()}{screen === 'progress' && Progress()}{screen === 'admin' && isAdmin && Admin()}{screen === 'auth' && Auth()}</SafeAreaView>;
}
