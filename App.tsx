...
export default function App() {
  ...
  function Home() { const memberRole = (user?.role as Role) ?? ROLES[0].name; return <ScrollView contentContainerStyle={styles.page}><View style={{ backgroundColor: '#F2F7FA', borderWidth: 1, borderColor: '#D9E6ED', borderLeftWidth: 5, borderLeftColor: COLORS.blue, borderRadius: 12, padding: compactHeader ? 20 : 26, gap: 11 }}><Text style={{ color: COLORS.blue, fontWeight: '900', fontSize: 12, letterSpacing: .6 }}>TCDD TAŞIMACILIK A.Ş. · SINAV HAZIRLIK PLATFORMU</Text><Text style={{ color: COLORS.ink, fontSize: compactHeader ? 27 : 31, fontWeight: '900', lineHeight: compactHeader ? 35 : 39 }}>Kariyerinde bir sonraki adıma hazır mısın?</Text><Text style={{ color: '#526576', fontSize: 16, lineHeight: 23 }}>Terfi Rotası ile eksiklerini gör, yolunu çiz. Unvanına özel konu çalışmaları, soru bankası ve sınav analizi tek yerde.</Text><Pressable onPress={() => user ? openStudy(memberRole) : (() => { setAuthMode('signup'); setAuthFeedback(null); setScreen('auth'); })()} style={{ alignSelf: compactHeader ? 'stretch' : 'flex-start', backgroundColor: COLORS.navy, paddingHorizontal: 21, paddingVertical: 13, borderRadius: 8, marginTop: 7, alignItems: 'center' }}><Text style={{ color: COLORS.white, fontSize: 14, fontWeight: '900' }}>{user ? 'HEMEN BAŞLA' : 'ROTANI OLUŞTUR'}</Text></Pressable></View><View style={{ flexDirection: 'row', gap: 9 }}><View style={styles.stat}><Text style={styles.statValue}>9</Text><Text style={styles.small}>Unvan</Text></View><View style={styles.stat}><Text style={styles.statValue}>11</Text><Text style={styles.small}>Sınav konusu</Text></View><View style={styles.stat}><Text style={styles.statValue}>{published}+</Text><Text style={styles.small}>Mevcut soru</Text></View></View><CurrentExamCards /><Footer /></ScrollView>; }
  function CurrentExamCards() { const showInfo = (selectedRole: Role) => { setInfoRole(selectedRole); setScreen('role-info'); }; const cardsPerPage = compactHeader ? 1 : 4; const cardWidth = compactHeader ? 245 : Math.max(275, Math.floor((windowWidth - 108) / 4)); const pageCount = Math.ceil(ROLES.length / cardsPerPage); const pageWidth = cardsPerPage * cardWidth + (cardsPerPage - 1) * 14; const moveCarousel = (direction: number) => { const nextPage = Math.max(0, Math.min(pageCount - 1, examCarouselPage + direction)); setExamCarouselPage(nextPage); examCarouselRef.current?.scrollTo({ x: nextPage * pageWidth, animated: true }); }; return <View style={{ padding: compactHeader ? 14 : 22, gap: 14, backgroundColor: COLORS.bg }}><View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}><Text style={styles.heading}>Güncel Sınavlar</Text>{compactHeader ? <Text style={{ color: COLORS.muted, fontSize: 12 }}>Kaydırarak incele →</Text> : <View style={{ flexDirection: 'row', gap: 8 }}><Pressable disabled={examCarouselPage === 0} onPress={() => moveCarousel(-1)} style={{ width: 38, height: 38, borderRadius: 19, borderWidth: 1, borderColor: examCarouselPage === 0 ? COLORS.line : COLORS.blue, alignItems: 'center', justifyContent: 'center', backgroundColor: examCarouselPage === 0 ? '#F4F7F9' : COLORS.white }}><Text style={{ color: examCarouselPage === 0 ? COLORS.muted : COLORS.blue, fontSize: 25, lineHeight: 27 }}>‹</Text></Pressable><Pressable disabled={examCarouselPage === pageCount - 1} onPress={() => moveCarousel(1)} style={{ width: 38, height: 38, borderRadius: 19, borderWidth: 1, borderColor: examCarouselPage === pageCount - 1 ? COLORS.line : COLORS.blue, alignItems: 'center', justifyContent: 'center', backgroundColor: examCarouselPage === pageCount - 1 ? '#F4F7F9' : COLORS.white }}><Text style={{ color: examCarouselPage === pageCount - 1 ? COLORS.muted : COLORS.blue, fontSize: 25, lineHeight: 27 }}>›</Text></Pressable></View>}</View><ScrollView ref={examCarouselRef} horizontal showsHorizontalScrollIndicator={false} onMomentumScrollEnd={event => { if (!compactHeader) setExamCarouselPage(Math.round(event.nativeEvent.contentOffset.x / pageWidth)); }} contentContainerStyle={{ paddingBottom: compactHeader ? 0 : 4 }}><View style={{ flexDirection: 'row', gap: 14 }}>{ROLES.map(item => <Pressable key={item.name} onPress={() => showInfo(item.name)} style={[styles.card, { width: cardWidth, minHeight: 270, alignItems: 'center', justifyContent: 'space-between' }]}><Image source={TCDD_LOGO} style={{ width: 142, height: 142, resizeMode: 'contain' }} /><Text style={{ textAlign: 'center', color: COLORS.ink, fontWeight: '800' }}>{item.name}</Text><Text style={{ textAlign: 'center', color: COLORS.muted, fontSize: 12 }}>{item.special}</Text><View style={styles.primaryButton}><Text style={styles.primaryText}>Konuları İncele</Text></View></Pressable>)}</View></ScrollView>{!compactHeader && <View style={{ flexDirection: 'row', justifyContent: 'center', gap: 7 }}>{Array.from({ length: pageCount }).map((_, dotIndex) => <View key={dotIndex} style={{ width: dotIndex === examCarouselPage ? 20 : 8, height: 8, borderRadius: 4, backgroundColor: dotIndex === examCarouselPage ? COLORS.blue : '#DCE5EB' }} />)}</View>}</View>; }
