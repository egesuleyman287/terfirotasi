...
export default function App() {
  ...
  function Home() { const memberRole = (user?.role as Role) ?? ROLES[0].name; return <ScrollView contentContainerStyle={styles.page}><View style={{ backgroundColor: '#F2F7FA', borderWidth: 1, borderColor: '#D9E6ED', borderLeftWidth: 5, borderLeftColor: COLORS.blue, borderRadius: 12, padding: compactHeader ? 17 : 25, gap: compactHeader ? 8 : 11 }}><Text style={{ color: COLORS.blue, fontWeight: '900', fontSize: 12, letterSpacing: .6 }}>SINAV HAZIRLIK PLATFORMU</Text><Text style={{ color: COLORS.ink, fontSize: compactHeader ? 25 : 31, fontWeight: '900', lineHeight: compactHeader ? 31 : 39 }}>Hedefindeki unvan için planlı hazırlan.</Text><Text style={{ color: '#526576', fontSize: compactHeader ? 14 : 16, lineHeight: compactHeader ? 20 : 23 }}>Unvanına özel konu çalışmaları, soru bankası ve sınav analizi tek yerde.</Text><Pressable onPress={() => user ? openStudy(memberRole) : (() => { setAuthMode('signup'); setAuthFeedback(null); setScreen('auth'); })()} style={{ alignSelf: compactHeader ? 'stretch' : 'flex-start', backgroundColor: COLORS.navy, paddingHorizontal: 21, paddingVertical: 12, borderRadius: 8, marginTop: compactHeader ? 3 : 7, alignItems: 'center' }}><Text style={{ color: COLORS.white, fontSize: 14, fontWeight: '900' }}>{user ? 'HEMEN BAŞLA' : 'ROTANI OLUŞTUR'}</Text></Pressable></View><View style={{ flexDirection: 'row', gap: 9 }}><View style={styles.stat}><Text style={styles.statValue}>9</Text><Text style={styles.small}>Unvan</Text></View><View style={styles.stat}><Text style={styles.statValue}>11</Text><Text style={styles.small}>Sınav konusu</Text></View><View style={styles.stat}><Text style={styles.statValue}>{published}+</Text><Text style={styles.small}>Mevcut soru</Text></View></View><CurrentExamCards /><Footer /></ScrollView>; }
