...
export default function App() {
  ...
  function Home() {
    ...
    return <ScrollView contentContainerStyle={[styles.page, { maxWidth: 1180, width: '100%', alignSelf: 'center' }]}>
      <View style={{ backgroundColor: COLORS.navy, borderRadius: 16, padding: compactHeader ? 20 : 34, flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: 26 }}><View style={{ flex: 1, minWidth: compactHeader ? 0 : 420, gap: 12 }}><Text style={{ color: '#A9DAF0', fontSize: 12, fontWeight: '900', letterSpacing: .8 }}>TCDD TAŞIMACILIK A.Ş. · GÖREVDE YÜKSELME</Text><Text style={{ color: COLORS.white, fontSize: compactHeader ? 28 : 36, fontWeight: '900', lineHeight: compactHeader ? 35 : 44 }}>Hedefindeki unvan için sistemli hazırlan.</Text><Text style={{ color: '#DDEFFC', fontSize: 16, lineHeight: 24 }}>Konu çalışmaları, güncel soru havuzu ve ayrıntılı sınav analizi tek platformda.</Text><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginTop: 6 }}><Pressable onPress={() => user ? openStudy(memberRole) : (() => { setAuthMode('signup'); setAuthFeedback(null); setScreen('auth'); })()} style={{ backgroundColor: COLORS.red, paddingHorizontal: 19, paddingVertical: 12, borderRadius: 7 }}><Text style={{ color: COLORS.white, fontWeight: '900' }}>{user ? 'ÇALIŞMAYA BAŞLA' : 'ÜCRETSİZ ÜYE OL'}</Text></Pressable><Pressable onPress={() => setScreen('exams')} style={{ borderWidth: 1, borderColor: '#FFFFFF66', paddingHorizontal: 19, paddingVertical: 12, borderRadius: 7 }}><Text style={{ color: COLORS.white, fontWeight: '800' }}>SINAVLARI İNCELE</Text></Pressable></View></View><View style={{ width: compactHeader ? '100%' : 220, alignItems: 'center', justifyContent: 'center', paddingVertical: 10 }}><Image source={TCDD_LOGO} style={{ width: 175, height: 155, resizeMode: 'contain' }} /><Text style={{ color: '#DDEFFC', fontSize: 12, fontWeight: '800', textAlign: 'center' }}>TCDD TAŞIMACILIK A.Ş.</Text></View></View>
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12 }}><View style={[styles.stat, { minWidth: compactHeader ? '46%' : 0 }]}><Text style={styles.statValue}>9</Text><Text style={styles.small}>Açık unvan</Text></View><View style={[styles.stat, { minWidth: compactHeader ? '46%' : 0 }]}><Text style={styles.statValue}>11</Text><Text style={styles.small}>Konu başlığı</Text></View><View style={[styles.stat, { minWidth: compactHeader ? '46%' : 0 }]}><Text style={styles.statValue}>{published}+</Text><Text style={styles.small}>Soru havuzu</Text></View></View>
      <View style={{ gap: 12 }}><View><Text style={styles.heading}>Terfi Rotası ile hazırla</Text><Text style={styles.cardText}>Sınava hazırlık sürecini dağınık notlardan tek bir plana taşı.</Text></View><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12 }}><View style={[styles.card, { flex: 1, minWidth: compactHeader ? '100%' : 250 }]}><Text style={{ color: COLORS.blue, fontWeight: '900' }}>01 · KONU ÇALIŞMASI</Text><Text style={styles.cardTitle}>Mevzuata odaklan</Text><Text style={styles.cardText}>Unvanına göre konuları seç, mevzuat metinlerine hızlıca ulaş.</Text></View><View style={[styles.card, { flex: 1, minWidth: compactHeader ? '100%' : 250 }]}><Text style={{ color: COLORS.blue, fontWeight: '900' }}>02 · SORU BANKASI</Text><Text style={styles.cardTitle}>Bilgini pekiştir</Text><Text style={styles.cardText}>Dört şıklı sorularla çalış; her cevapta doğru seçenek ve dayanağı gör.</Text></View><View style={[styles.card, { flex: 1, minWidth: compactHeader ? '100%' : 250 }]}><Text style={{ color: COLORS.blue, fontWeight: '900' }}>03 · ANALİZ</Text><Text style={styles.cardTitle}>Gelişimini izle</Text><Text style={styles.cardText}>Sınav sonunda doğru-yanlışlarını ve konu bazlı başarı durumunu incele.</Text></View></View></View>
      <View style={[styles.card, { backgroundColor: '#F3F9FC', borderColor: '#B9D8E8', flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 14 }]}><View style={{ flex: 1, minWidth: 220 }}><Text style={styles.cardTitle}>Unvanının sınav kapsamını incele</Text><Text style={styles.cardText}>Sınavlar sayfasında sorumlu konuları ve çalışma içeriklerini görebilirsin.</Text></View><Pressable style={styles.primaryButton} onPress={() => setScreen('exams')}><Text style={styles.primaryText}>SINAVLARA GİT</Text></Pressable></View><Footer />
    </ScrollView>;
  }
  function CurrentExamCards() {
    const accents = [COLORS.blue, COLORS.navy, '#126F9A', COLORS.blue, COLORS.navy, '#126F9A', COLORS.blue, COLORS.navy, '#126F9A'];
    const showInfo = (selectedRole: Role) => { setInfoRole(selectedRole); setScreen('role-info'); };
    return <View style={{ padding: compactHeader ? 12 : 18, gap: 14, backgroundColor: COLORS.bg }}><View style={{ backgroundColor: COLORS.white, borderWidth: 1, borderColor: COLORS.line, borderRadius: 13, padding: compactHeader ? 14 : 18, flexDirection: 'row', alignItems: 'center', gap: 14 }}><Image source={TCDD_LOGO} style={{ width: compactHeader ? 58 : 72, height: compactHeader ? 52 : 64, resizeMode: 'contain' }} /><View style={{ flex: 1 }}><Text style={styles.heading}>Güncel Sınavlar</Text><Text style={{ color: COLORS.muted, fontSize: 12, marginTop: 3 }}>TCDD Taşımacılık A.Ş. görevde yükselme sınavları</Text></View><View style={{ backgroundColor: '#EAF5FA', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 8 }}><Text style={{ color: COLORS.blue, fontWeight: '900', fontSize: 12 }}>{ROLES.length} UNVAN</Text></View></View><Text style={{ color: COLORS.muted, fontSize: 13 }}>Unvanını seçerek sorumlu konuları ve çalışma kapsamını inceleyebilirsin.</Text><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 14 }}>{ROLES.map((item, roleIndex) => { const accent = accents[roleIndex % accents.length]; const initials = item.name.split(' ').filter(word => word.length > 2).slice(0, 2).map(word => word[0]).join(''); return <Pressable key={item.name} onPress={() => showInfo(item.name)} style={[styles.card, { width: compactHeader ? '100%' : '31.8%', minWidth: compactHeader ? 0 : 250, minHeight: compactHeader ? 164 : 185, padding: 0, overflow: 'hidden' }]}><View style={{ height: 5, backgroundColor: accent }} /><View style={{ padding: 16, flex: 1, gap: 10, justifyContent: 'space-between' }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}><View style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: `${accent}16`, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: accent, fontWeight: '900', fontSize: 15 }}>{initials}</Text></View><View style={{ flex: 1 }}><Text style={{ color: COLORS.muted, fontSize: 10, fontWeight: '800' }}>TCDD TAŞIMACILIK A.Ş.</Text><Text style={{ color: COLORS.ink, fontWeight: '900', fontSize: 15, lineHeight: 20 }}>{item.name}</Text></View></View><Text style={{ color: COLORS.muted, fontSize: 12, lineHeight: 17 }}>{item.special}</Text><View style={{ alignSelf: 'flex-start', backgroundColor: `${accent}14`, borderRadius: 6, paddingHorizontal: 11, paddingVertical: 7 }}><Text style={{ color: accent, fontWeight: '900', fontSize: 12 }}>Konuları incele →</Text></View></View></Pressable>; })}</View></View>;
  }
