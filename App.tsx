...
export default function App() {
  ...
  function Home() {
    ...
    return <ScrollView contentContainerStyle={[styles.page, { gap: compactHeader ? 14 : 20 }]}>
      ...
      {user && <View style={{ backgroundColor: '#F1F8FC', borderWidth: 1, borderColor: '#C9E1EE', borderRadius: 14, padding: compactHeader ? 15 : 18, gap: 13 }}><View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}><View style={{ flex: 1, minWidth: 210, gap: 4 }}><Text style={{ color: COLORS.blue, fontWeight: '900', fontSize: 12, letterSpacing: .6 }}>SANA ÖZEL HAZIRLIK ALANI</Text><Text style={{ color: COLORS.ink, fontSize: compactHeader ? 19 : 21, fontWeight: '900' }}>Hoş geldin, {user.name.split(' ')[0]}.</Text><Text style={{ color: COLORS.muted, lineHeight: 19 }}>{user.institution ?? 'TCDD Taşımacılık A.Ş.'} · {user.role ?? 'Unvanın seçilmedi'}</Text></View><Pressable onPress={() => setScreen('auth')} style={{ borderWidth: 1, borderColor: '#A9D1E5', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 9 }}><Text style={{ color: COLORS.blue, fontSize: 12, fontWeight: '900' }}>PROFİLİM</Text></Pressable></View><View style={{ flexDirection: 'row', gap: 10 }}><View style={{ flex: 1, backgroundColor: COLORS.white, borderRadius: 10, padding: 12, gap: 3 }}><Text style={{ color: COLORS.blue, fontWeight: '900', fontSize: 19 }}>{plan === 'premium' ? 'Sınırsız' : `${Math.max(0, 10 - freeTopicUsed)}/10`}</Text><Text style={{ color: COLORS.muted, fontSize: 12 }}>Kalan soru hakkı</Text></View><View style={{ flex: 1, backgroundColor: COLORS.white, borderRadius: 10, padding: 12, gap: 3 }}><Text style={{ color: plan === 'premium' || !freeMockUsed ? COLORS.green : COLORS.red, fontWeight: '900', fontSize: 19 }}>{plan === 'premium' ? 'Sınırsız' : freeMockUsed ? 'Kullanıldı' : 'Hazır'}</Text><Text style={{ color: COLORS.muted, fontSize: 12 }}>Genel deneme hakkı</Text></View></View><Pressable onPress={goToStart} style={[styles.primaryButton, { alignSelf: compactHeader ? 'stretch' : 'flex-start', minWidth: compactHeader ? 0 : 230 }]}><Text style={styles.primaryText}>ÇALIŞMAYA BAŞLA</Text></Pressable></View>}
